...
export const editorialProductionScriptRetryCapacities = sqliteTable(
  'editorial_production_script_retry_capacities',
  {
    legacyAttestationId: text('legacy_attestation_id').notNull(),
    id: text('id').primaryKey(),
    workspaceId: text('workspace_id').notNull(),
    projectId: text('project_id').notNull(),
    revisionRequestId: text('revision_request_id').notNull(),
    expectedProjectVersion: integer('expected_project_version').notNull(),
    requiredProjectStatus: text('required_project_status').notNull(),
    failedRunId: text('failed_run_id').notNull(),
    failedAttemptId: text('failed_attempt_id').notNull(),
    failedReservationId: text('failed_reservation_id').notNull(),
    failedEnvelopeId: text('failed_envelope_id').notNull(),
    failedExecutionIdempotencyKey: text('failed_execution_idempotency_key').notNull(),
    failedActualMicrousd: integer('failed_actual_microusd').notNull(),
    briefArtifactId: text('brief_artifact_id').notNull(),
    briefVersionId: text('brief_version_id').notNull(),
    briefApprovalId: text('brief_approval_id').notNull(),
    briefContentHash: text('brief_content_hash').notNull(),
    expectedBriefArtifactRevision: integer('expected_brief_artifact_revision').notNull(),
    scriptArtifactId: text('script_artifact_id').notNull(),
    expectedCurrentScriptVersionId: text('expected_current_script_version_id').notNull(),
    expectedScriptArtifactRevision: integer('expected_script_artifact_revision').notNull(),
    expectedScriptContentHash: text('expected_script_content_hash').notNull(),
    budgetId: text('budget_id').notNull(),
    envelopeId: text('envelope_id').notNull(),
    economicProfileKey: text('economic_profile_key').notNull(),
    economicProfileVersion: integer('economic_profile_version').notNull(),
    boundedProfileKey: text('bounded_profile_key').notNull(),
    boundedProfileVersion: integer('bounded_profile_version').notNull(),
    stageKey: text('stage_key').notNull(),
    monetaryCeilingMicrousd: integer('monetary_ceiling_microusd').notNull(),
    maximumCalls: integer('maximum_calls').notNull(),
    maximumAttempts: integer('maximum_attempts').notNull(),
    ...
    creativeRegenerationEnabled: integer('creative_regeneration_enabled').notNull(),
    externalToolsEnabled: integer('external_tools_enabled').notNull(),
    externalResearchEnabled: integer('external_research_enabled').notNull(),
    humanApprovalRequired: integer('human_approval_required').notNull(),
    providerId: text('provider_id').notNull(),
    providerModelId: text('provider_model_id').notNull(),
    promptVersionId: text('prompt_version_id').notNull(),
...
const productionScriptRetrySchemaObjects = [
  ...
  'production_script_retry_attestation_binding',
  ...
  'production_script_retry_eligible_policy',
  'production_script_retry_execution_eligible',
  'production_script_retry_capacity_scope_guard',
  'production_script_retry_capacity_policy_guard',
  'production_script_retry_capacity_actor_guard',
  'production_script_retry_capacity_budget_guard',
  'production_script_retry_capacity_result_guard',
  'production_script_retry_capacity_audit_guard',
  ...
  'production_script_retry_capacity_budget_immutable',
  'production_script_retry_capacity_envelope_immutable',
] as const;
const productionScriptRetryRequiredColumns = [
  'legacy_attestation_id',
  'id',
  'workspace_id',
  'project_id',
  'revision_request_id',
...
// Rechecked inside the same atomic batch as every legacy authorization/claim/dispatch.
export const productionScriptRetrySchemaPredicate = `(SELECT count(*) FROM sqlite_master WHERE name IN (${productionScriptRetrySchemaObjects.map((name) => "'" + name + "'").join(',')}))=${productionScriptRetrySchemaObjects.length}
 AND (SELECT count(*) FROM pragma_table_info('editorial_production_script_retry_capacities') WHERE name IN (${productionScriptRetryRequiredColumns.map((name) => "'" + name + "'").join(',')}))=${productionScriptRetryRequiredColumns.length}`;
export async function productionScriptRetrySchemaReady(db: D1Database) {
  try {
    const objects = (
      await db
        .prepare(
          `SELECT name FROM sqlite_master WHERE name IN (${productionScriptRetrySchemaObjects.map(() => '?').join(',')})`,
        )
        .bind(...productionScriptRetrySchemaObjects)
        .all<{ name: string }>()
    ).results;
    if (
      !productionScriptRetrySchemaObjects.every((name) => objects.some((row) => row.name === name))
    )
      return false;
    const columns = (
      await db
        .prepare("PRAGMA table_info('editorial_production_script_retry_capacities')")
        .all<{ name: string }>()
    ).results;
    return productionScriptRetryRequiredColumns.every((name) =>
...
export const editorialLegacyRemediationAttestations = sqliteTable(
  'editorial_legacy_remediation_attestations',
  {
    id: text('id').primaryKey(),
    incidentCode: text('incident_code').notNull(),
    observedState: text('observed_state').notNull(),
    evidenceGap: text('evidence_gap').notNull(),
    ...
    schemaCapabilityVersion: integer('schema_capability_version').notNull(),
    ...
    terminalAuditId: text('terminal_audit_id').notNull(),
    evidenceJson: text('evidence_json').notNull(),
...
export const editorialLegacyRemediationClaims = sqliteTable('editorial_legacy_remediation_claims', {
  id: text('id').primaryKey(),
  attestationId: text('attestation_id').notNull(),
  capacityId: text('capacity_id').notNull(),
