...
export function createStoryboardSceneV2Schema(allowedSegmentIds?: readonly string[]) {
  if (allowedSegmentIds !== undefined) {
    if (allowedSegmentIds.length === 0)
      throw new Error('Storyboard source Script segments cannot be empty.');
    if (new Set(allowedSegmentIds).size !== allowedSegmentIds.length)
      throw new Error('Storyboard source Script segments cannot contain duplicate IDs.');
  }
  const segmentId =
    allowedSegmentIds && allowedSegmentIds.length > 0
      ? z.enum(allowedSegmentIds as [string, ...string[]])
      : id;
  return z
    .object({
      order: z.number().int().positive(),
      targetDurationSeconds: z.number().positive().nullable(),
      scriptSegmentIds: z.array(segmentId).min(1).max(20),
      ...
      captions: z
        .object({
          ...
          languageCode: languageCodeSchema,
          sourceScriptSegmentIds: z.array(segmentId).max(20),
...
export function createStoryboardOutputV2Schema(allowedSegmentIds?: readonly string[]) {
  const sceneSchema = createStoryboardSceneV2Schema(allowedSegmentIds);
  return z
    .object({
...
export const storyboardSceneV2Schema = createStoryboardSceneV2Schema();
export const storyboardOutputV2Schema = createStoryboardOutputV2Schema();
...
export const governedStageCapacitySchema = z
  .object({
    budgetId: id,
    expectedBudgetVersion: z.number().int().positive(),
    stageKey: z.literal('STORYBOARD_PLANNER'),
    reason: z.literal('OPEN_REVISION_STORYBOARD_REPLACEMENT'),
  })
  .strict();
export type GovernedStageCapacityCommand = z.infer<typeof governedStageCapacitySchema>;
