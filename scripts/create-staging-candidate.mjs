...
if (!/^[0-9a-f]{40}$/u.test(sha ?? ''))
  throw new Error('Usage: pnpm staging:candidate -- <source-sha> [--enable-openai-connectivity]');
if (mode !== undefined && mode !== '--enable-openai-connectivity')
  throw new Error('Unsupported candidate mode');
...
const enabled = mode === '--enable-openai-connectivity' ? 'true' : 'false';
run('pnpm', ['build:web']);
run('pnpm', [
  'exec',
  'wrangler',
  ...
  'RELEASE_VERSION:' + sha,
  '--var',
  'OPENAI_PROVIDER_ENABLED:' + enabled,
  '--var',
  'AI_PROVIDER_CONNECTIVITY_DIAGNOSTIC_ENABLED:' + enabled,
  '--message',
