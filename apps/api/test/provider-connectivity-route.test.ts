...
function setup(role: 'owner' | 'admin' | 'operator') {
  const writes: string[] = [];
  const db = {
  ...
  const checkConnectivity = vi.fn().mockResolvedValue({
    ok: true,
...
describe('provider connectivity admin route', () => {
  it.each(['owner', 'admin'] as const)(
    'allows %s and creates only a safe audit event',
    async (role) => {
      ...
      expect(writes[0]).toContain('INSERT INTO audit_events');
      expect(writes[0]).not.toMatch(/intelligence_runs|editorial_artifacts|artifact_versions/);
    },
  );
